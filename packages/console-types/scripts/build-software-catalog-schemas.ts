...
const comment = 'This file was automatically generated, do not modify it by hand. Instead, modify the source Typescript file, and run `pnpm types build:software-catalog-schemas`.'

const outDir = path.resolve(process.cwd(), 'schemas/software-catalog')
...
const buildWellKnownItemsSchemas = async(): Promise<void> => {
  const buildManifest = async(data: CatalogWellKnownItemData): Promise<void> => {
    ...
    manifest = set(lensPath(['$id']), `software-catalog-${data.type}.manifest.schema.json`, manifest)
    manifest = set(lensPath(['title']), `Software Catalog ${data.type} item manifest`, manifest)
    manifest = set(lensPath(['description']), `The data needed to apply a single version of a Software Catalog ${data.type} item.`, manifest)
  ...
  const buildItem = async(data: CatalogWellKnownItemData): Promise<void> => {
    ...
    item = set(lensPath(['$id']), `software-catalog-${data.type}.item.schema.json`, item)
    item = set(lensPath(['title']), `Software Catalog ${data.type} item`, item)
    item = set(lensPath(['description']), `A single version of a Software Catalog ${data.type} item.`, item)
  ...
  const buildResources = async(data: CatalogWellKnownItemData): Promise<void> => {
    ...
    resources = set(lensPath(['$id']), `software-catalog-${data.type}.resources.schema.json`, resources)
    resources = set(lensPath(['title']), `Software Catalog ${data.type} item resources`, resources)
    resources = set(lensPath(['description']), `The resources of a Software Catalog ${data.type} item.`, resources)
